...
<?php include "dbCon.php";
	...
	$sql = "Select room_no,room_cost,room_type from  (hall_room natural join room) natural join hall where room_status != 'Occupied' and hall_name='".$hall_name."'";
	if($is_query_run= mysql_query($sql,$conn)){
						
				echo "<br>";
				
				echo '<table cellpadding="0" cellspacing="0" class="db-table">';
				echo '<th>Index No</th><th>Room No</th><th>Room Cost</th><th>Room Type</th>';
		...
		while($row = mysql_fetch_array($is_query_run,MYSQL_ASSOC)){
         	...
         	echo '<tr>';
	   	 	echo '<td>'.++$count.'</td><td>'.$row['room_no'].'</td><td>'.$row['room_cost'].'</td><td>'.$row['room_type'].'</td>';
			echo '</tr>';
