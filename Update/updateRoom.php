...
if(test_input(empty($_POST['room_cost']))){
...
if(test_input(empty($_POST['room_type']))){
	$errors="error-complete all fields";
	echo "<script> alert('error-complete all fields')</script>";
	echo "<script> window.history.go(-1)</script>";
}else{
...
if(test_input(empty($_POST['room_status']))){
	$errors="error-complete all fields";
	echo "<script> alert('error-complete all fields')</script>";
	echo "<script> window.history.go(-1)</script>";
}else{
$room_status=test_input($_POST['room_status']);
}
if(!$errors){
//Execute the query 
mysqli_query($link,"UPDATE room SET room_cost='$room_cost', room_type='$room_type', room_status='$room_status' WHERE room_no='$room_no'");
if(mysqli_affected_rows($link) > 0){
	echo "<h3>Room infomation updated successfully.</h3>";
}else{
	echo "<h3>Room infomation has not been changed.</h3>";
}	
}
}